...
const shouldRenderCricApi = isBrowser && !isAdmin;

if (shouldRenderCricApi) {
  document.addEventListener('DOMContentLoaded', () => {
    const wrapper = document.getElementById('matches-wrapper') || document.getElementById('matchesWrapper');
    if (!wrapper) {
    ...
    function renderCricAPIMatches() {
      ...
      matches.slice(0, 5).forEach((match) => {
        ...
        const name =
          match.name ||
          match.match ||
          `${match.teams?.[0] ?? match.teamA ?? 'Team A'} vs ${match.teams?.[1] ?? match.teamB ?? 'Team B'}`;
